type HaftarahInfo = {
  ...
  function convertToHebrewText(haftarah: string): string {
    return haftarah.replace(/(\w+)\s(\d+):(\d+)(?:-?(\d+):?)?(\d+)?/g, (_, book, p1, v1, p2, v2) => {
      const bookHebrew = bookNamesMap[book] || book;
  ...
  function convertToHebrewDescription(data: HaftarahInfo): string {
    ...
    if (data.haftarah) {
      sections.push(`📖 הפטרה (אשכנז): ${convertToHebrewText(data.haftarah)}`);
    }
    if (data.haftarah_sephardic) {
      sections.push(`📖 הפטרה (ספרדי): ${convertToHebrewText(data.haftarah_sephardic)}`);
    }
    if (data.haftarah_yemenite) {
      sections.push(`📖 הפטרה (תימני): ${convertToHebrewText(data.haftarah_yemenite)}`);
    }
    if (data.haftarah_chabad) {
      sections.push(`📖 הפטרה (חב"ד): ${convertToHebrewText(data.haftarah_chabad)}`);
    }
